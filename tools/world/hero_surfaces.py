...
field/=np.max(np.abs(field));height=field*.0006
...
(out/'material.json').write_text(json.dumps({'source':'original seeded analytic aggregate microrelief','seed':7182,'resolution':1024,'repeat_m':2,'height_amplitude_m':.0006,'normal':'OpenGL','roughness':'authored .63-.87','maps':'linear data; aggregate is an artistic tint field','credits':0},indent=2))
