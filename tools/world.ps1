param([ValidateSet('validate','preview','launch','test-gameplay','preview-playable')][string]$Action='validate',[ValidateSet('clear','rain','heavy_rain','fog','cloudy','storm')][string]$Weather='clear',[ValidateRange(0,4)][int]$Quality=2)
...
if($Action -in @('test-gameplay','preview-playable')) {
